...
describe("processStats", () => {
  ...
  test("ignores non-JS assets", () => {
    const stats = makeStats({ "app/about/page": { assets: [{ name: "page.css" }] } }, [
  ...
  test('strips "app" prefix and "/page" suffix from route name', () => {
    const stats = makeStats({ "app/about/page": { assets: [{ name: "about.js" }] } }, [
  ...
  test('maps root route to "/"', () => {
    const stats = makeStats({ "app/page": { assets: [{ name: "index.js" }] } }, [
  ...
  test("sums multiple JS assets for a route", () => {
    const stats = makeStats({ "app/blog/page": { assets: [{ name: "a.js" }, { name: "b.js" }] } }, [
      { name: "a.js", size: 1000 },
      { name: "b.js", size: 500 },
    ]);
  ...
  test("calls getGzipSize for each JS asset and accumulates result", () => {
    const stats = makeStats({ "app/shop/page": { assets: [{ name: "a.js" }, { name: "b.js" }] } }, [
      { name: "a.js", size: 2000 },
      { name: "b.js", size: 3000 },
    ]);
    const calls = [];
    const getGzipSize = (name) => {
      calls.push(name);
      return 100;
    };
  ...
  test("handles asset as plain string (not object)", () => {
    const stats = makeStats({ "app/page": { assets: ["home.js"] } }, [
...
describe("processNewStats", () => {
  ...
  test("single route: all chunks are shared, route is filtered out", () => {
    const stats = makeNewStats([["/", 1000, ["a.js", "b.js"]]]);
  ...
  test("skips entries without route name", () => {
    const stats = [{ firstLoadUncompressedJsBytes: 1000, firstLoadChunkPaths: ["a.js"] }];
    const routes = processNewStats(stats, () => 100);
...
describe("parseStatsFile", () => {
  ...
  test("resolves chunk paths from stats file location (new format, .next/ prefix)", () => {
    ...
    fs.writeFileSync(path.join(chunksDir, "about.js"), "// about");
  ...
  test("logs a warning and returns gzip 0 when stats path is outside a .next directory", () => {
    ...
    try {
      const routes = parseStatsFile(statsPath, true);
      assert.ok(
        logs.some((m) => m.includes("Could not find .next directory in path")),
        "expected warning about missing .next",
      );
      assert.ok(
        routes["global"] !== undefined || Object.keys(routes).length === 0,
        "should not throw",
      );
    } finally {
  ...
  test("resolves app-paths-manifest.json via dotNextDir for route group mapping", () => {
    ...
    const routes = parseStatsFile(statsPath, true);
    assert.ok(
      "/(frontend)/book-session/[hash]" in routes,
      "route group prefix should be restored from manifest",
    );
    assert.ok(
      !("/book-session/[hash]" in routes),
      "bare route should be replaced by route-group-prefixed key",
    );
  });
});
...
const REPORT_HEADER_WITH_APP = (name) =>
  `## 📦 ${name} — App Router Sizes (Turbopack)\n\nThis analysis was generated by the [Next.js Turbopack Bundle Size action](https://github.com/michalsanger/nextjs-turbopack-bundle-size). 🤖\n\n`;
...
describe("generateReport full table", () => {
  ...
  test("all routes unchanged", () => {
    ...
    assert.equal(
      ...
      REPORT_HEADER + "This PR introduced no changes to the JavaScript bundle! 🙌\n",
    );
  });
  ...
  test("uses app name in header when provided", () => {
    assert.equal(
      generateReport({ "/": { gzip: 512 } }, {}, 0, 0, "My App"),
      REPORT_HEADER_WITH_APP("My App") +
        "| Route | Size (gzipped) | First load | Diff (vs baseline) |\n" +
        "|---|---|---|---|\n" +
        "| `/` | `512 B` | `512 B` | 🆕 New |\n",
    );
  ...
  test("uses default header when app name is empty", () => {
    assert.equal(
      generateReport({ "/": { gzip: 512 } }, {}, 0, 0, ""),
      REPORT_HEADER +
        "| Route | Size (gzipped) | First load | Diff (vs baseline) |\n" +
        "|---|---|---|---|\n" +
        "| `/` | `512 B` | `512 B` | 🆕 New |\n",
