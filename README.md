...
| `baseline-workflow`           | No       | —                                           | Workflow file name (e.g. `deploy.yml`) to look up the baseline artifact in. Set this when the baseline artifact is uploaded by a different workflow than the one running this action. When unset, the artifact is searched in the current workflow.                                                                        |
